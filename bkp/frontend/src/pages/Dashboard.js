        ...
        <div className="card">
          ...
          <div className="card-content">
            <ResponsiveContainer width="100%" height={300}>
              <RechartsPieChart>
                <Pie
                  data={Array.isArray(resumoCategorias) ? resumoCategorias.filter(cat => cat.valor > 0) : []}
                  cx="50%"
                  cy="50%"
                  outerRadius={80}
                  dataKey="valor"
        ...
        <div className="card-content">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.isArray(resumoPlataformas) ? resumoPlataformas.map((plataforma, index) => (
              <div key={plataforma.nome} className="border border-slate-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="font-semibold text-slate-900">{plataforma.nome}</h4>
                  <div className={`h-3 w-3 rounded-full`} style={{ backgroundColor: plataforma.cor || COLORS[index % COLORS.length] }}></div>
                </div>
