...
const Perfil = () => {
  ...
  const { register: registerPerfil, handleSubmit: handleSubmitPerfil, reset: resetPerfil, formState: { errors: errorsPerfil } } = useForm({
    defaultValues: {
      nome: user?.nome || '',
      email: user?.email || '',
  ...
  const atualizarPerfil = async (data) => {
    try {
      // Converter pais_id para número se existir
      const dadosProcessados = {
        ...data,
        pais_id: data.pais_id ? parseInt(data.pais_id) : null
      };
      
      const response = await configuracaoService.atualizarPerfil(dadosProcessados);
      updateUser(response);
                ...
                <form onSubmit={handleSubmitPerfil(atualizarPerfil)} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <select {...registerPerfil('pais_id')} className="input">
